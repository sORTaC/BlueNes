...
uint16_t NesPPU::ppu_read(int addr) 
{
	...
	//	if (horizontal_mirroring)
	//	{
	//		return ppu_ram[addr & 0x2400];
	//	}
	//	else
	//	{
	//		return ppu_ram[addr & 0x2800];
	//	}
	//}
	//else
		return ppu_ram[addr]; 
...
void NesPPU::step_ppu()
{
    ...
    else if (ppu_scanline == 241 && ppu_cycles == 1)
    {
        ...
        busPtr->set_nmi();
		printf("TILE : %x\n", ppu_read(0x23E2));
		//for (int i = 0; i < 0x400; i += 0x20)
		//{
		//	for (int j = 0; j < 0x20; j++)
		//		printf("%x ", ppu_read(0x2000 + j + i));
		//	printf("\n");
		//}
        drawNametable();
    }
...
void NesPPU::drawNametable()
{
	...
	SDL_Texture* texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_STATIC, 256, 240);

	Uint32* pixels = new Uint32[960 * 256];
